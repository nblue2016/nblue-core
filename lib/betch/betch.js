...
class Betch
{
  ...
  static run (val, ctx, data) {
    const callback =
      (err, reject) => Betch.catchRuntimeError(ctx, err, flagOfUnhandledErrors)
      ...
      then((result) => {
        ...
        if (errors && Object.keys(errors).length > 0) {
          ...
          if (!ctx[flagOfIgnoreError]) {
            const err = new Error('runtime error of betch')
      ...
      if (Array.isArray(val)) {
        ...
        const gen = function *() {
          ...
          for (const key of keys) {
            ...
            if (isNested) newCtx[flagOfFullReturn] = true
            ...
            try {
              ...
              if (isNested) newCtx[flagOfFullReturn] = false
            }
          }
  ...
  static getVal (key, val, ctx, data) {
    ...
    const catchError = (err) => {
      ...
      return ctx[flagOfCatchError] === true
        ? Promise.reject(err)
    ...
    try {
      ...
      if (typeof result === 'object' &&
        ...
        return result.catch(
          (promiseError) => catchError(promiseError)
        )
      }
  ...
  static parseVal (key, val, ctx, data) {
    ...
    if (key.endsWith('$')) {
      result = Betch.run(result, ctx, data)
    }
  ...
  static catchRuntimeError (ctx, err, flag) {
    // get flag in error collection
    const newFlag = flag || flagOfRuntimeErrors

    if (ctx && ctx[flagOfErrors]) {
      // get errors from context
      const errors = ctx[flagOfErrors]

      // create new array for current flag
      if (!errors[newFlag]) {
        errors[newFlag] = []
      }

      // push error to current flag
      errors[newFlag].push(err)
    }
  }
    ...
    const gen = function *() {
      ...
      if (fdata === null) {
        return Promise.reject(
          new Error(`read script file: ${file} failed`)
        )
      }
